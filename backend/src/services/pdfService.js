...
function buildInvoiceHtml(invoiceData, businessProfile = {}, templateId = 'modern') {
    ...
    const currency = invoiceData.currency || businessProfile.currency || 'INR';
  ...
  <main class="page">
    <section class="topbar">
      <div>
        ...
        ${businessName ? `<h1 class="business-name">${escapeHtml(businessName)}</h1>` : ''}
      </div>
    ...
    <section class="bottom">
      ...
      <section class="summary">
        ...
        <div class="total-row"><span>Subtotal:</span><span>${formatINR(invoiceData.subtotal, currency)}</span></div>
        <div class="total-row"><span>Tax:</span><span>${formatINR(invoiceData.taxTotal, currency)}</span></div>
        <div class="grand-total"><span>Total (${escapeHtml(currency)}):</span><span>${formatINR(invoiceData.total, currency)}</span></div>
...
function buildFallbackPdf(invoiceData, businessProfile, templateId = 'modern', isDemo) {
    ...
    let page = createPage();
    pages.push(page);
    if (isDemo) drawDemoWatermark(page, fonts);
    ...
    for (let index = 0; index < items.length; index += 1) {
        ...
        page.cursorY = drawItemRow(page, index, item, page.cursorY, fonts.regular, theme, currency);
    }
    ...
    page.cursorY -= 12;
    page.cursorY = drawBottomSummary(page, invoiceData, businessProfile, page.cursorY, fonts, theme, currency);
...
function drawDetailCards(page, y, invoiceData, businessProfile, fonts, theme) {
    ...
    const cardY = y - 84;
    const cardHeight = 92;
    const businessName = businessProfile?.businessName || businessProfile?.name || 'Invoice';
    const clientCompany = invoiceData.clientCompanyName || invoiceData.company || invoiceData.clientDetails?.companyName;
    const clientGst = invoiceData.clientGstNumber || invoiceData.clientDetails?.gstNumber;
    const clientAddress = invoiceData.clientAddress || invoiceData.clientDetails?.address;
...
function drawPrimaryHeader(page, invoiceData, businessProfile, isDemo, fonts, theme) {
    ...
    if (invoiceData.status) {
        detailY = drawText(page, safeText(`Status: ${invoiceData.status}`), 350, detailY, 10, fonts.regular, { color: theme.muted });
    }
...
function drawContinuationHeader(page, invoiceData, businessProfile, fonts, theme) {
    ...
    y = drawText(page, safeText(businessProfile?.businessName || businessProfile?.name || 'Invoice'), PAGE_MARGIN, y, 16, fonts.bold, { color: theme.text });
    if (invoiceData.number) {
...
function drawItemRow(page, index, item, y, font, theme, currency = 'INR') {
...
function drawBottomSummary(page, invoiceData, businessProfile, y, fonts, theme, currency = 'INR') {
    const leftX = PAGE_MARGIN;
    const summaryX = 335;
    const notes = invoiceData.notes || '';
    ...
    y -= 8;
    y = drawAmountRow(page, 'Subtotal:', formatFallbackMoney(invoiceData.subtotal, currency), y, fonts.regular, fonts.bold, false, summaryX, theme);
    y = drawAmountRow(page, 'Tax:', formatFallbackMoney(invoiceData.taxTotal, currency), y, fonts.regular, fonts.bold, false, summaryX, theme);
    y -= 4;
    y = drawAmountRow(page, `Total (${currency}):`, formatFallbackMoney(invoiceData.total, currency), y, fonts.bold, fonts.bold, true, summaryX, theme);
    ...
    drawText(page, 'Authorized Signatory', summaryX, Math.max(PAGE_MARGIN + 4, y - 58), 9, fonts.bold, { color: theme.text });
    return y - 72;
}
...
function formatFallbackMoney(amount, currency = 'INR') {
